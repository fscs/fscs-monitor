...
pub async fn get_list_of_semesters() -> Vec<Semester>{
    ...
    let table = html.select(&scraper::Selector::parse("table").unwrap()).next().unwrap();
    ...
    let names: Vec<String> = regex.captures_iter(&table.html()).map(|x| x[1].to_string()).collect();

    //use regex to get start and end date of semester
    let dates = regex::Regex::new(r"(\d{2}.\d{2}.\d{4}) – (\d{2}.\d{2}.\d{4})").unwrap(); 
    let dates: Vec<(String, String)> = dates.captures_iter(&table.html()).map(|x| (x[1].to_string(), x[2].to_string())).collect();
